...
package object io {
  ...
  def readFile(log: Logger, file: File)(implicit ctx: PluginContext): Either[Throwable, String] = {
    log.pluginDebug(s"Attempting to read ${file.getAbsolutePath}")
    Try(IO.read(file, UTF_8)).toEither
  }

  def writeFile(log: Logger, outputFile: File, content: String)(implicit ctx: PluginContext): Unit = {
  ...
  def verifyNonEmptyFileExists(log: Logger, file: File, isEmptyLine: String => Boolean, failureHelp: String)(
      implicit ctx: PluginContext
  ): Unit = {
